...
public class VehiculeResourceIT {
     ...
     *
     * @see com.pfe.covite.repository.search.VehiculeSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createVehicule() throws Exception {
        ...
        verify(mockVehiculeSearchRepository, times(1)).save(testVehicule);
    }
    ...
    public void createVehiculeWithExistingId() throws Exception {
        ...
        // Validate the Vehicule in Elasticsearch
        verify(mockVehiculeSearchRepository, times(0)).save(vehicule);
    }
    ...
    @Test
    @Transactional
    public void getAllVehiculesByCommandeIsEqualToSomething() throws Exception {
        // Initialize the database
        vehiculeRepository.saveAndFlush(vehicule);
        Commande commande = CommandeResourceIT.createEntity(em);
        em.persist(commande);
        em.flush();
        vehicule.addCommande(commande);
        vehiculeRepository.saveAndFlush(vehicule);
        Long commandeId = commande.getId();

        // Get all the vehiculeList where commande equals to commandeId
        defaultVehiculeShouldBeFound("commandeId.equals=" + commandeId);

        // Get all the vehiculeList where commande equals to commandeId + 1
        defaultVehiculeShouldNotBeFound("commandeId.equals=" + (commandeId + 1));
    }
    ...
    public void updateVehicule() throws Exception {
        ...
        reset(mockVehiculeSearchRepository);
    ...
    public void deleteVehicule() throws Exception {
        ...
        // Validate the Vehicule in Elasticsearch
        verify(mockVehiculeSearchRepository, times(1)).deleteById(vehicule.getId());
    }
    ...
    public void searchVehicule() throws Exception {
        // Initialize the database
        vehiculeService.save(vehicule);
        when(mockVehiculeSearchRepository.search(queryStringQuery("id:" + vehicule.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(vehicule), PageRequest.of(0, 1), 1));
        // Search the vehicule
